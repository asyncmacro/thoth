  ...
  return async (request: Request) => {
    ...
    try {
      ...
      if (url.pathname === "/version" && request.method === "GET") {
        return new Response(JSON.stringify({ version: env.VERSION ?? "0.1.0" }), {
          status: 200,
      ...
      if (vaultIdMatch) {
        ...
        if (url.pathname === `/vaults/${vaultId}` && request.method === "DELETE") {
          if (env.VAULT_DO) {
            const doId = env.VAULT_DO.idFromName(vaultId);
            const stub = env.VAULT_DO.get(doId);
        ...
        if (url.pathname.startsWith(`/vaults/${vaultId}/devices`)) {
          ...
          if (url.pathname === `/vaults/${vaultId}/devices` && request.method === "POST") {
          ...
          if (url.pathname === `/vaults/${vaultId}/devices` && request.method === "GET") {
          ...
          const deviceActionMatch = url.pathname.match(new RegExp(`/vaults/${vaultId}/devices/([^/]+)(/.*)?`));
